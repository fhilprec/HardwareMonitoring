...
// Check no Memory Leak and Measured Values are consistent across time
int main() {
    //config
    int minutesForProcess = 60;
    std::string outputFolder = "test_long_time";
    ...
    auto currentTime = std::chrono::duration_cast<std::chrono::minutes>(std::chrono::system_clock::now() - start);
    while (currentTime < std::chrono::minutes(minutesForProcess))
    {
        doCPUWorkFlops();
        currentTime = std::chrono::duration_cast<std::chrono::minutes>(std::chrono::system_clock::now() - start);
    }

    monitor.stop();

    std::cout << "Test completed. Check the" << outputFolder << " directory for results." << std::endl;
