...
static const std::vector METRICS{
    Metric(TWO_SHOT, "cycles", true),
    Metric(TWO_SHOT, "kcycles", true),
    Metric(TWO_SHOT, "instructions", true),
    Metric(TWO_SHOT, "L1-misses", true),
    Metric(TWO_SHOT, "LLC-misses", true),
    Metric(TWO_SHOT, "branch-misses", true),
    Metric(TWO_SHOT, "task-clock", true)
};

CPUPerfTwoShot::CPUPerfTwoShot(const std::vector<Metric>& metricsToCount): Device(metricsToCount)
{
...
void CPUPerfTwoShot::registerCounter(uint64_t type, uint64_t eventID)
{
    events.emplace_back();
...
std::vector<std::pair<Metric, Measurement>> CPUPerfTwoShot::getData(const SamplingMethod sampler)
{
...
Measurement CPUPerfTwoShot::fetchMetric(const Metric& metric)
{
...
Measurement CPUPerfTwoShot::calculateMetric(const Metric& metric,
                                     ...
                                     size_t timeIndexForMetric)
{
    ...
    parseData(requestedMetricsByDeviceBySamplingMethod.at(getDeviceName()).at(TWO_SHOT).at(false).at(0), metric, valuePrev, timeEnabledPrev, timeRunningPrev);

    uint64_t valueAfter, timeEnabledAfter, timeRunningAfter;
    parseData(requestedMetricsByDeviceBySamplingMethod.at(getDeviceName()).at(TWO_SHOT).at(false).at(1), metric, valueAfter, timeEnabledAfter, timeRunningAfter);
...
void CPUPerfTwoShot::parseData(const std::unordered_map<Metric, Measurement>& row, const Metric& rawMetric, uint64_t &value,
...
std::unordered_map<std::string, Metric> CPUPerfTwoShot::getAllDeviceMetricsByName() {
...
std::unordered_map<std::string, std::vector<Metric>> CPUPerfTwoShot::getNeededMetricsForCalculatedMetrics(const Metric& metric)
{
...
std::string CPUPerfTwoShot::getDeviceName() {
    return "CPUPerfTwoShot";
}